...
LinkedListMethod::LinkedListMethod(TypeDictionary *d)
   : MethodBuilder(d)
   {
   ...
   DefineName("search");
   IlType *pElementType = d->PointerTo((char *)"Element");
...
class LinkedListTypeDictionary : public TypeDictionary
   {
   public:
   LinkedListTypeDictionary() :
      TypeDictionary()
      {
      IlType *ElementType = DEFINE_STRUCT(Element);
      IlType *pElementType = PointerTo("Element");
