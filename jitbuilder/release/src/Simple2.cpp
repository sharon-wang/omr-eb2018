...
main(int argc, char *argv[])
   ...
   SimpleMethod method(&types, "increment");
   uint8_t *entry = 0;
   ...
   {
   cout << "Step 5: define type dictionary\n";
   TR::TypeDictionary types;

   cout << "Step 6: compile method builder\n";
   SimpleMethod method(&types, "add_one");
   uint8_t *entry = 0;
   int32_t rc = compileMethodBuilder(&method, &entry);
   if (rc != 0)
      {
   ...
   cout << "Step 7: invoke compiled code and print results\n";
   typedef int32_t (SimpleMethodFunction)(int32_t);
   SimpleMethodFunction *add_one = (SimpleMethodFunction *) entry;
...
print(int32_t v)
   {
   cout << "(result for " << v << " is) ";
   }

SimpleMethod::SimpleMethod(TR::TypeDictionary *d, const char *name)
   : MethodBuilder(d)
   {
   ...
   DefineFunction("print", "Simple.cpp", "101", (void *)&::print, NoType, 1, Int32);
   }
